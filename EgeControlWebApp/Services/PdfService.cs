...
namespace EgeControlWebApp.Services
{
            ...
            var bytes = Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Content().Column(col =>
                    {
                        ...
                        col.Item().PaddingTop(12).AlignRight().Width(300).Column(totalCol =>
                        {
                            // Üst mavi başlık
                            totalCol.Item().Padding(8).Background("#4A90E2").AlignCenter()
                                .Text("GENEL TOPLAM").Bold().FontSize(14).FontColor("#FFFFFF");
                            
                            // Alt beyaz alan - Toplam tutar
                            totalCol.Item().Border(2).BorderColor("#4A90E2").Padding(15).Background("#FFFFFF").AlignCenter()
                                .Text($"TOPLAM: {CurrencyHelper.FormatCurrency(quote.TotalAmount, quote.Currency ?? "TRY")}")
                                .Bold().FontSize(18).FontColor("#FF0000");
                        });
                        ...
                        col.Item().PaddingTop(8).AlignRight().Column(signatureCol =>
                        {
                            try
                            {
                                ...
                                if (File.Exists(imagePath) && new FileInfo(imagePath).Length > 0)
                                {
                                    ...
                                    // Resmi başarılı şekilde yükleyebiliyorsak, esnek boyutlarla kullan
                                    signatureCol.Item()
                                        .MaxWidth(120)
                                        .MaxHeight(80)
                                        .AlignCenter()
                                ...
                                {
                                    // Dosya yoksa veya boşsa text fallback kullan
                                    signatureCol.Item()
                                        .AlignCenter()
                                        .Text("N. ÖZDEMİR")
                                        .FontSize(12)
                            ...
                            {
                                // Herhangi bir hata durumunda text fallback kullan
                                signatureCol.Item()
                                    .AlignCenter()
                                    .Text("N. ÖZDEMİR")
                                    .FontSize(12)
                        ...
                        {
                            col.Item().PaddingTop(6).Text("NOTLAR").Bold();
                            col.Item().Text(quote.Notes);
                        }
                    });
    ...
    public Task<byte[]> GenerateCustomerListPdfAsync(IEnumerable<Customer> customers)
        {
            var data = customers.Where(c => c.IsActive).ToList();
            var bytes = Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    {
                        col.Item().Text("EGE CONTROL - MÜŞTERİ LİSTESİ").SemiBold().FontSize(18).FontColor(Colors.Blue.Medium).AlignCenter();
                        col.Item().Text($"Rapor Tarihi: {DateTime.Now:dd.MM.yyyy HH:mm}").FontSize(10).AlignCenter();
                    ...
                    page.Content().Table(table =>
                    {
                        ...
                        table.Header(header =>
                        {
                            header.Cell().Element(H).Text("Şirket Adı");
                            header.Cell().Element(H).Text("İletişim Kişisi");
                            header.Cell().Element(H).Text("Email");
                            header.Cell().Element(H).Text("Telefon");
                            header.Cell().Element(H).Text("Şehir");
                            header.Cell().Element(H).Text("Kayıt Tarihi");

                            static IContainer H(IContainer c) => c.DefaultTextStyle(x => x.SemiBold()).PaddingVertical(4).BorderBottom(1).BorderColor(Colors.Grey.Lighten1);
                        });

                        foreach (var c in data)
                        {
                            table.Cell().Text(c.CompanyName);
                            table.Cell().Text(c.ContactPerson);
                            table.Cell().Text(c.Email);
                            table.Cell().Text(c.Phone);
                            table.Cell().Text(c.City);
                            table.Cell().Text(c.CreatedAt.ToString("dd.MM.yyyy"));
                        }
                    });

                    page.Footer().AlignRight().Text($"Toplam Müşteri Sayısı: {data.Count}").SemiBold();
                });
            }).GeneratePdf();
        ...
        private string GetStatusText(QuoteStatus status)
        {
            return status switch
            {
                ...
                QuoteStatus.Accepted => "Onaylandı",
